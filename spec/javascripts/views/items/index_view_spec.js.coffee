describe "Actions.Views.Items.IndexView", ->
  ...
  describe "Rendering", ->
    ...
    it "create a item view for each item", ->
      ...
      expect(@editViewStub).toHaveBeenCalledWith({ model: @item1, template: @template, inList: true })
      expect(@editViewStub).toHaveBeenCalledWith({ model: @item2, template: @template, inList: true })
